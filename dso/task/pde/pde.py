...
class PDETask(HierarchicalTask):
    ...
    def reward_function(self,p):
        # import pdb;pdb.set_trace()
        y_hat_init, _, w = p.execute(self.u, self.x, self.ut)
        ...
        FW = np.multiply(self.x[0], self.x[3])
        try:
            y_hat_init_r = y_hat_init.reshape(200, 100, -1).squeeze()
            y_hat = np.matmul(y_hat_init_r.T, FW) + self.x[4]
    ...
    def mse_function(self,p):
        y_hat_init, y_right, w = p.execute(self.u, self.x, self.ut)
        # y_hat, y_right, w = p.execute(self.u, self.x, self.ut)
        try:
            FW = np.multiply(self.x[0], self.x[3])
            y_hat = np.matmul(y_hat_init.T, FW) + self.x[4]
            diffs = y_hat - self.ut
            # loss = (np.mean(np.abs(diffs)))
    ...
    def evaluate(self, p):
        ...
        if p.invalid:
            ...
            # nmse_test = np.mean(np.abs(self.ut - y_hat))
