...
class Program(object):
    ...
    def execute_BFGS(self, u, x, ut,):
        ...
        self.set_stridge()
        self.STRidge.terms_token_constant_left = self.traversal
        token_inx = 0
        for token in self.traversal:
            if token == self.library.tokens[0]:
                self.STRidge.terms_token_constant_right.append(self.library.tokens[1])
            elif token == self.library.tokens[1]:
                self.STRidge.terms_token_constant_right.append(self.library.tokens[0])
            else:
                self.STRidge.terms_token_constant_right.append(token)
            token_inx += 1

        y_hat_left, w_best, self.invalid, self.error_node, self.error_type, y_right = self.STRidge.evaluate(self.STRidge.terms_token_constant_left,
                                                                                                       u, x, ut,
                                                                                                       Program.execute_function)
        y_hat_right, w_best, self.invalid, self.error_node, self.error_type, y_right = self.STRidge.evaluate(self.STRidge.terms_token_constant_right,
                                                                                                       u, x, ut,
                                                                                                       Program.execute_function)
        y_hat =  y_hat_left * (u[0] <= u[1]).astype(int) + y_hat_right * (u[0] > u[1]).astype(int)
