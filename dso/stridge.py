...
class STRidge(object):
    ...
    def __init__(self, traversal, default_terms =[], noise_level=0,
        ...
        self.terms_right = []
        self.terms_left = []
        self.terms_token_left = []
        self.terms_token_right = []
        self.terms_token_constant_left = []
        self.terms_token_constant_right = []
        self.invalid = False
        self.w_sym = []
    ...
    def split_forest(self):
        ...
        def expand_list_left(input_list):
            """ expand multiple lists to one list"""
            while len(input_list) != 0:
                cur = input_list.pop(0)
                if isinstance(cur, list):
                    expand_list_left(cur)
                else:
                    self.terms_left.append(cur)

        def expand_list_right(input_list):
            """ expand multiple lists to one list"""
            while len(input_list) != 0:
                cur = input_list.pop(0)
                if isinstance(cur, list):
                    expand_list_right(cur)
                else:
                    self.terms_right.append(cur)
        ...
        if len(term_list) == 2:
            term_list_left = term_list_copy[0]
            term_list_right = term_list_copy[1]
            expand_list_left(term_list_left)
            expand_list_right(term_list_right)
            # self.terms_token_left = preorder_traverse(root.children[0])
            # self.terms_token_right = preorder_traverse(root.children[1])

        # if len(root.children) == 2:
        #     self.terms_token_constant_left = preorder_traverse(root.children[0])
        #     self.terms_token_constant_right = preorder_traverse(root.children[1])
        ...
        self.terms_token_left = [preorder_traverse(node) for node in self.terms_left]
        self.terms_token_right = [preorder_traverse(node) for node in self.terms_right]

        self.terms_len = [len(tokens) for tokens in self.terms_token]
        self.terms_len_left = [len(tokens) for tokens in self.terms_token_left]
        self.terms_len_right = [len(tokens) for tokens in self.terms_token_right]
    ...
    def calculate(self, u,x,ut, test=False, execute_function = unsafe_execute, cached=None):
        ...
        if not isinstance(ut, list):
            ...
            return self.coef_calculate(results,ut,x)
        else:
            #stridge
    ...
    def evaluate_terms(self,u,x, test=False, execute_function = unsafe_execute):
        ...
        for i, traversal in enumerate(self.terms_token_left):
            result, invalid, error_node, error_type = execute_function(traversal, u, x)
            # result = result[2:-2,1:] #RRE
            if invalid:
                return 0, [0], invalid, error_node, error_type, None
            ...
            if torch.is_tensor(result):
                ...
                if self.noise_level > 0:
                    result = cut_bound(result, percent=self.cut_ratio, test=test)
            result = (u[0] <= u[1]).astype(int) * result
            results.append(result.reshape(-1))
            # results.append(result)

        for i, traversal in enumerate(self.terms_token_right):
            result, invalid, error_node, error_type = execute_function(traversal, u, x)
            # result = result[2:-2,1:] #RRE
            if invalid:
                return 0, [0], invalid, error_node, error_type, None
    ...
    def coef_calculate(self, rhs, lhs, x):
        # from sklearn.linear_model import LinearRegression
        # lr = LinearRegression(fit_intercept=False).fit(rhs,lhs)
        try:
            # w_best = np.linalg.lstsq(rhs, lhs)[0]
            n_term = rhs.shape[1]
            rhh = rhs.reshape(200, 100, -1)
            FW = np.multiply(x[0], x[3])
            Rr = np.zeros((100, 100, n_term))
            for inx in range(n_term):
                Rr[:, :, inx] = np.matmul(rhh[:, :, inx].T, FW)+x[4]
            Rr = Rr.reshape(-1, n_term)
            lhs = lhs.reshape(-1, 1)
            w_best = np.linalg.lstsq(Rr, lhs)[0]
        except Exception as e:
            # print(e)
            invalid = True
