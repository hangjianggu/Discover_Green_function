...
def make_prior(library, config_prior):
    ...
    prior_dict = {
        ...
        "compare_des":compare_des,
        "compare_constraint":compare_constraint,
        "nest_des":nest_des
    }
...
class compare_des(RelationalConstraint):
    def __init__(self, library, extra = None):
        not_included_names = [name for name in library.names if library.names.index(name) not in library.terminal_tokens]
        if library.const_token is not None:
        ...
        effectors = []
        for operator_name in library.names:
            if 'large' in operator_name or 'less' in operator_name:
                effectors.append(operator_name)
        super(compare_des, self).__init__(library=library,
                                             targets=targets,
                                             effectors=effectors,
                                             relationship="child")

class nest_des(RelationalConstraint):
    def __init__(self, library, extra = None):
        not_included_names = ["sin", "sinh", "exp"]
        targets = [library.names.index(name) for name in not_included_names if name in library.names]
        effectors = []
        for operator_name in library.names:
            if 'sin' in operator_name or 'sinh' in operator_name or 'exp' in operator_name:
                effectors.append(operator_name)
        super(nest_des, self).__init__(library=library,
                                             targets=targets,
                                             effectors=effectors,
                                             relationship="descendant")

class exp_des(RelationalConstraint):
    def __init__(self, library, extra = None):
        not_included_names = [name for name in library.names if library.names.index(name) not in library.terminal_tokens]
        if library.const_token is not None:
            not_included_names.append('const')
        ...
        effectors = []
        for operator_name in library.names:
            if 'exp' in operator_name:
                effectors.append(operator_name)
        super(exp_des, self).__init__(library=library,
                                             targets=targets,
                                             effectors=effectors,
                                             relationship="child")

class compare_constraint(RelationalConstraint):
    def __init__(self, library, extra = None):
        not_included_names = ['large', 'less']
        if extra is not None:
            for name  in extra:
                not_included_names.append(name)
        targets  = [library.names.index(name) for name in not_included_names]
        effectors = []
        for operator_name in library.names:
            if library.names.index(operator_name) not in library.terminal_tokens and operator_name!='mul':
                effectors.append(operator_name)
        super(compare_constraint, self).__init__(library=library,
                                             targets=targets,
                                             effectors=effectors,
                                             relationship="child")
